...
| 대상 | 모으는 것·설정 |
|---|---|
| EKS 컴퓨터와 Pod 일꾼 | CPU·메모리 등 사용량. 300초(5분) 간격으로 모으도록 설정했어요 |
...
Container Insights 수집 주기는 비용 절감을 위해 60초에서 300초로 늘렸습니다. 경보와 대시보드의 기존 300초 평가 간격에 맞춘 값입니다. 짧은 CPU·메모리 급증은 놓칠 수 있으며, 수집 지연으로 빈 구간이 생기는지 적용 후 확인해야 합니다. 앱 로그 수집과 다른 AWS 서비스의 지표 주기는 이 설정으로 바뀌지 않습니다. 저장소 변경만으로 실제 수집 주기가 바뀌지는 않으므로 아래 Terraform 적용 후 실행 중인 Agent 설정과 새 지표의 간격을 확인합니다.
