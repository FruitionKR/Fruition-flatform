...
| EC2 일반 노드 | Cluster Autoscaler가 자원 부족으로 배치되지 못한 Pod를 보고 조절 | 최소 2대, 최대 3대 |
| EC2 AI 노드 | 같은 방식, CPU Spot 사용 | 최소 0대, 최대 2대 |

트래픽 증가만으로 일반 API의 Pod 수가 늘어나지는 않습니다. AI 작업 Pod가 늘어나도 EC2 최대치에 도달하면 대기할 수 있습니다. CloudWatch는 관측·알림용이며 현재 API HPA와 연결되지 않습니다. Pod 2개를 실행할 여유가 없으면 일반 노드가 기존 상한인 3대까지 늘어나 비용이 증가할 수 있습니다.
